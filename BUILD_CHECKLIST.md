...
> ## Implementation status — 228 requirements built
>
> `[~]` = **implemented in code, not yet run on a device.** The build
> environment had no Flutter SDK (pub.dev and storage.googleapis.com are
> blocked from this sandbox), so nothing here has been compiled or executed.
> The logic that is expensive to get wrong — the checklist text matcher and
> the anti-resurrection decision table — was validated against a reference
> implementation and has Dart unit tests ready to run.
>
> `[ ]` on a **QA-** item = waiting for you to run it on Windows/Android.
> `[ ]` on a **LATER** item = deliberately out of scope, do not build.
>
